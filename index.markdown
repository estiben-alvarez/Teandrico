...
<div style="position:relative;overflow:hidden;margin-bottom:3rem;">
      ...
      <div class="article-list-item" style="background:var(--color-background-primary,#fff);border:0.5px solid var(--color-border-tertiary);border-radius:var(--border-radius-lg);overflow:hidden;">
        ...
        <a href="{{ post.url }}" style="display:block;position:relative;height:260px;overflow:hidden;">
          ...
          <div style="position:absolute;bottom:0;left:0;right:0;padding:1rem;background:linear-gradient(to top,rgba(26,26,26,0.92),transparent);">
            {% if post.categories %}
            <span style="font-size:11px;letter-spacing:0.08em;text-transform:uppercase;background:#f5c842;color:#1a1a1a;padding:2px 8px;border-radius:3px;display:inline-block;margin-bottom:0.4rem;">{{ post.categories | first }}</span>
            {% else %}
            <span style="font-size:11px;letter-spacing:0.08em;text-transform:uppercase;background:#f5c842;color:#1a1a1a;padding:2px 8px;border-radius:3px;display:inline-block;margin-bottom:0.4rem;">Noticias</span>
            {% endif %}
            <p style="font-family:Georgia,serif;font-size:1rem;font-weight:normal;margin:0;line-height:1.3;color:#fff;">{{ post.title }}</p>
          </div>
        </a>
        {% endif %}
        <div style="padding:0.8rem 1rem;">
          <p style="font-size:0.78rem;color:var(--color-text-secondary);margin:0 0 0.5rem;">{{ post.date | date: "%B %d, %Y" }}</p>
          <p style="font-size:0.9rem;color:var(--color-text-secondary);margin:0 0 0.8rem;line-height:1.5;">{{ post.excerpt | strip_html | truncate: 150 }}</p>
  ...
  <button onclick="moverCarrusel(-1)" style="position:absolute;left:0;top:130px;background:#f5c842;border:none;padding:0.5rem 1rem;cursor:pointer;font-size:1.2rem;z-index:10;color:#1a1a1a!important;">‹</button>
  <button onclick="moverCarrusel(1)" style="position:absolute;right:0;top:130px;background:#f5c842;border:none;padding:0.5rem 1rem;cursor:pointer;font-size:1.2rem;z-index:10;color:#1a1a1a!important;">›</button>
</div>
